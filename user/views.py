    ...
    if request.method != 'POST':
        ...
        if name and password and password_repeat:
            if User.objects.filter(name=name):
                return response(PARAMS_ERROR, '用户名已存在！', True)
            if password != password_repeat:
                return response(PARAMS_ERROR, '两次密码不一致！', True)
            salt = create_salt()
            password_encode = create_md5(password, salt)
            User.objects.create(name=name, password=password_encode, salt=salt)
            return response(SUCCESS, '注册成功！')
        else:
            return response(PARAMS_ERROR, '提交字段名不可为空！', True)
    ...
    if request.method != 'GET':
        ...
        if value:
            ...
            try:
                ...
                return response(MYSQL_ERROR, '用户不存在！', True)
        else:
        ...
        if value:
            ...
            try:
                ...
                if scholar_id:
                    ...
                    return response(PARAMS_ERROR, '学者id不能为空', True)
            except Exception as e:
                return response(MYSQL_ERROR, '用户不存在！', True)
        else:
            return response(PARAMS_ERROR, 'token错误！', True)
